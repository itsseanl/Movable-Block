...
const BlockManifestScript = dirname("..", "/scripts/build-blocks-manifest");
			...
			cacheGroups: {
				style: {
					...
					name(_, chunks, cacheGroupKey) {
						...
						return `${dirname(chunkName)}/${cacheGroupKey}-${basename(
							chunkName,
						)}`;
					},
				},
		...
		apply(compiler) {
			if (this.blockJsonFiles.length) {
				compiler.hooks.compilation.tap(
					...
					(compilation) => {
						compilation.fileDependencies.addAll(this.blockJsonFiles);
					},
				);
			}
		}
